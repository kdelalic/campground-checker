...
- `campsite_checker/dashboard.py` — Static HTML dashboard generation. Produces a self-contained HTML file (no external assets) with availability tables per campground. Entries whose search failed (`ProcessedAvailability.search_succeeded=False`) render as an amber "Scan failed" card rather than as "No availability", so a provider cooldown is never mistaken for an empty campground; the fingerprint in `results.py` includes `search_succeeded` so a failure→success transition rewrites the page. Each date row is a `<details>` disclosure listing individual sites with per-site booking links, and the page reloads itself every `DEFAULT_REFRESH_SECONDS` (300) while the tab is visible.
