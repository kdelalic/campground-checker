...
The Dockerfile defaults to `python check_campsites.py --forever --dashboard` and uses Python 3.14-slim. The container runs as the unprivileged user `app` (uid 10001); any mounted volume the checker writes (e.g. the `state/` directory) must be writable by that uid on the host. Runtime tunables via env vars: `WORKERS` (default 4), `ALERT_INTERVAL` (default 1), `SEARCH_DELAY` (default 1), `BATCH_SIZE` (default 4), `DASHBOARD_INTERVAL` (default 10), `GC_INTERVAL` (default 12), `THROTTLE_BASE_DELAY` (default 30 seconds), and `THROTTLE_MAX_DELAY` (default 900 seconds). `docker-compose.yml` overrides `WORKERS=8` and `BATCH_SIZE=2` (host `.env` can override further): camply issues one request per campground per month sequentially within a batch, so smaller batches and more workers cut dashboard scan wall-clock; dial back toward the image defaults if provider throttle cooldowns start firing. Set `SENT_KEYS_PATH` to persist dedup keys across container restarts (e.g. a mounted volume). `CAMPLY_CACHE_DIR` relocates camply's UseDirect offline metadata cache (image default `/app/state/camply-cache`; outside Docker it defaults to `.camply-cache/`) — camply would otherwise write it into site-packages, which the unprivileged user cannot do. `docker-compose.yml` defines a healthcheck against the health endpoint, so a stale checker is reported as an unhealthy container.
